...
public class MainActivity extends AppCompatActivity {
    ...
    private void openHome() {
        Intent intent = new Intent(this.getApplicationContext(), DaftarLagu.class);
//        intent.putExtra("username", username);
        this.startActivity(intent);
    }

    private void saveCredentials() {
        SharedPreferences handler = this.getSharedPreferences("MainActivity",Context.MODE_PRIVATE);
    ...
    private boolean checkCredentials(String username, String password) {
        if (username.equals("rossi") && password.equals("rossi")) {
            return true;
        } else {
            return false;
        }
